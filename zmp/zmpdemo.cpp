...
/*TODO remove this
 */
const int stance_foot_table[4] =
{ 0, 1, 0, 1 };
const int swing_foot_table[4] =
{ -1, -1, 1, 0 };

const stance_t next_stance_table[4] =
{ SINGLE_LEFT, SINGLE_RIGHT, DOUBLE_RIGHT, DOUBLE_LEFT };
...
public:
	...
	ZmpDemo(int argc, char** argv, HuboPlus& h, const TrajVector& t) :
			MzGlutApp(argc, argv,
					GLUT_DOUBLE | GLUT_DEPTH | GLUT_RGB | GLUT_MULTISAMPLE), hplus(
					h), kbody(h.kbody), traj(t), cur_index(-1)
	...
	virtual void deltaCurrent(int delta)
	{
		...
		for (int i = 0; i < delta; ++i)
		{
			...
			if (new_stance_foot != stance_foot)
			{
				...
				stance_foot_xform = state.xform()
						* kbody.manipulatorFK(xforms, stance_foot);
			}
	...
	virtual void display()
	{
		...
		glPushMatrix();
		glTranslated(actualCom[0], actualCom[1],
				actualCom[2] + hplus.footAnkleDist);
...
void validateCOMTraj(Eigen::MatrixXd& comX, Eigen::MatrixXd& comY)
{
	...
	double comStateMaxes[] =
	{ 0.0, 0.0, 0.0 };
	const double comStateTol[] =
	{ 2.0, 5.0 }; // m/s, m/s^2, m/s^3
	for (int n = 0; n < (comX.rows() - 1); n++)
	{
		// Calculate COM vel and acc norms from x and y components
		comVel = sqrt(
				(comX(n + 1, 0) - comX(n, 0)) * (comX(n + 1, 0) - comX(n, 0))
						+ (comY(n + 1, 0) - comY(n, 0))
								* (comY(n + 1, 0) - comY(n, 0))) / dt;
		comAcc = sqrt(
				(comX(n + 1, 1) - comX(n, 1)) * (comX(n + 1, 1) - comX(n, 1))
						+ (comY(n + 1, 1) - comY(n, 1))
								* (comY(n + 1, 1) - comY(n, 1))) / dt;
		// Update max state values
		if (comVel > comStateMaxes[0])
			comStateMaxes[0] = comVel;
		if (comAcc > comStateMaxes[1])
			comStateMaxes[1] = comAcc;
		// Check if any are over limit
		if (comVel > comStateTol[0])
		{
			std::cerr << "COM velocity sample " << n + 1 << "is larger than "
					<< comStateTol[0] << "(" << comVel << ")\n";
		}
		if (comAcc > comStateTol[1])
		{
			std::cerr << "COM acceleration of sample " << n + 1
					<< "is larger than " << comStateTol[1] << "(" << comAcc
					<< ")\n";
		}
	}
	std::cerr << "comMaxVel: " << comStateMaxes[0] << "\ncomMaxAcc: "
			<< comStateMaxes[1] << std::endl;
...
void validateOutputData(TrajVector& traj)
{
	...
	for (int n = 0; n < (int) (traj.size() - 1); n++)
	{
		for (int j = 0; j < HUBO_JOINT_COUNT; j++)
		{
			...
			{
				std::cerr << "change in joint " << j << "is larger than "
						<< jointVelTol << "(" << jointVel << ")\n";
			}
...
enum walktype
{
	walk_canned, walk_line, walk_circle
};
...
int main(int argc, char** argv)
{
	...
	walktype walk_type = walk_circle;
	double walk_circle_radius = 2.0;
	double walk_dist = 20;
	...
	ZMPWalkGenerator::ik_error_sensitivity ik_sense =
			ZMPWalkGenerator::ik_strict;
			...
			&option_index)) != -1)
	{
		...
		case 'A':
			...
			break;
		case 'I':
			ik_sense = getiksense(optarg);
			break;
		case 'w':
			walk_type = getwalktype(optarg);
			break;
		case 'D':
			walk_dist = getdouble(optarg);
			break;
		case 'r':
			walk_circle_radius = getdouble(optarg);
			break;
		case 'c':
			max_step_count = getlong(optarg);
			break;
		case 'y':
			footsep_y = getdouble(optarg);
			break;
		case 'z':
			foot_liftoff_z = getdouble(optarg);
			break;
		case 'l':
			step_length = getdouble(optarg);
			break;
		case 'S':
			walk_sideways = true;
			break;
		case 'h':
			com_height = getdouble(optarg);
			break;
		case 'a':
			com_ik_ascl = getdouble(optarg);
			break;
		case 'Y':
			zmpoff_y = getdouble(optarg);
			break;
		case 'X':
			zmpoff_x = getdouble(optarg);
			break;
		case 'T':
			lookahead_time = getdouble(optarg);
			break;
		case 'p':
			startup_time = getdouble(optarg);
			break;
		case 'n':
			shutdown_time = getdouble(optarg);
			break;
		case 'd':
			double_support_time = getdouble(optarg);
			break;
		case 's':
			single_support_time = getdouble(optarg);
			break;
		case 'R':
			zmp_jerk_penalty = getdouble(optarg);
			break;
		case 'H':
			usage(std::cout);
			exit(0);
	...
	// the actual state
	ZMPWalkGenerator walker(hplus, ik_sense, com_height, zmp_jerk_penalty,
			zmpoff_x, zmpoff_y, com_ik_ascl, single_support_time,
			double_support_time, startup_time, shutdown_time, foot_liftoff_z,
			lookahead_time);
	...
	initContext.feet[0] = Transform3(starting_location.rotation(),
			starting_location * vec3(0, footsep_y, 0));
	initContext.feet[1] = Transform3(starting_location.rotation(),
			starting_location * vec3(0, -footsep_y, 0));
	...
	case walk_circle:
	{
		...
		std::cout << "Walking Circle." << std::endl;
		footprints = walkCircle(walk_circle_radius, walk_dist, footsep_y,
				step_length, circle_max_step_angle, &initLeftFoot,
				&initRightFoot, false);
	...
	case walk_line:
	{
		...
		footprints = walkLine(walk_dist, footsep_y, step_length, &initLeftFoot,
				&initRightFoot, false);
	...
	default:
	{
		...
		{
			bool is_left = i % 2;
			if (walk_sideways && step_length < 0)
			{
				is_left = !is_left;
			}
			...
			footprints.push_back(
					Footprint(cur_x[swing], cur_y[swing], 0, is_left));
		}
	...
	{
		std::cout << "Resizing Prints..." << std::endl;
	...
	for (std::vector<Footprint>::iterator it = footprints.begin();
	...
	if (use_ach)
	{
		...
		trajectory.count = N;
		for (int i = 0; i < N; i++)
			memcpy(&(trajectory.traj[i]), &(walker.traj[i]),
					sizeof(zmp_traj_element_t));
	...
	if (show_gui)
	{
		...
		for (unsigned i = 0; i < footprints.size(); i++)
		{
			footprints[i] = Footprint(
					footprints[i].x(),
					footprints[i].y() - footsep_y,
					footprints[i].theta(),
					footprints[i].is_left);
		}
		demo.prints = footprints;
		demo.run();
